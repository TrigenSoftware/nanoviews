...
- `context$(context)(child)` with an `InjectionContext` instance (from `nanoviews/store`) builds the child within that very instance: it sees the instance's values and those of the parent it was created with, not the current context, and what the child resolves stays in it. This is how a context made outside the view is handed in, so the view and the code around it share instances: `const context = new InjectionContext([provide(Theme$, $theme)])`, `mount(() => context$(context)(App()), root)`, then `inject(Store$, context)` outside is the store the view uses.
